...
test_that("dimensions/type/output", {
  ...
  testthat::expect_error(get_var_sequence(iris, subj = "setosa", subj_var = "Species", var = "beauty"))
  testthat::expect_error(get_var_sequence(iris, subj = "obama", subj_var = "Species", var = "Sepal.Length"))
  ...
  testthat::expect_error(is.matrix(slice_var_sequence(sequence = c(1,2), lags = 3, label_length = 1, label_output = FALSE)))
  
  testthat::expect_length(get_var_array(data = alsfrs_data, subj_var = "subjid", var = "p5", time_var ="visdy",  label_length = 1, label_output = TRUE, lags = 3), 2)
  testthat::expect_true(is.data.frame(get_var_array(data = alsfrs_data, subj_var = "subjid", var = "p5", time_var ="visdy",  label_length = 1, label_output = FALSE, lags = 3)))
